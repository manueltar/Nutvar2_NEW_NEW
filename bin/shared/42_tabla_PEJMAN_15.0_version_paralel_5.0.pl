...
foreach my $ENSG_tok(sort keys %{$hash_ENSG{$SYMBOL_tok}})
{
	...
	foreach my $CCDS_tok(sort keys %{$hash_CCDS{$ENSG_tok}})
	{
		$hash_total_isoforms{$SYMBOL_tok}{$CCDS_tok}=1;
	}
}
}
...
{
	#
	my @total_CCDS=sort keys%{$hash_total_isoforms{$SYMBOL_tok}};
	my $total_CCDS = scalar(@total_CCDS);
...
my $NMD_derived_def="NaN";
		...
		if(exists($hash2{$CHROM_tok}{$SYMBOL_tok}{$POS_tok}{$REF_tok}{$ALT_tok}{$Effect_tok_definitive}{$CCDS_tok}{'NMD'}))
		{
		...
		if(exists($hash2{$CHROM_tok}{$SYMBOL_tok}{$POS_tok}{$REF_tok}{$ALT_tok}{$Effect_tok_definitive}{$CCDS_tok}{'NMD_derived'}))
		{
...
$NMD_ratio_hash{$CCDS_tok}{$NMD_def}{$NMD_derived_def}=1;
#~ print "SEGUNDO_NMD_ENST_a_ENST:$CCDS_tok\t$NMD_def\t$NMD_derived_def\t$SYMBOL_tok\n";
