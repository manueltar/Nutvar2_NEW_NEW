##	Script to add the layer of gene-based features to the matrix of data to use in the classifier. 2014.
...
if (open(OUTPUT, '>'.$output))
{
	print OUTPUT "baseNCBI37\tAlleles_REF>ALT\tGeneNameHGNC\tNumIsoformsInQueryGene\tratioIsoformsBearingTheVariant\t".
				"ratioAffectedIsoforms_stop_gained\tratioAffectedIsoforms_frameshift\tratioAffectedIsoforms_splice\tratioAffectedIsoforms_coding-synonymous\tratioAffectedIsoforms_missense\t".
				...
				"IsInnateImmunity\tIsAntiviral\tIsISG\tIsOMIMrecessive\tpRDG_score\tRVIS_score\n";
	...
	#~ print OUTPUT "#X:74334588\tC>T\tABCB7\t6\t4\t".
				...
				#~ "1\t100\t100\t1\t1\t".
				#~ "1\t100\t100\t1\t0\t".
				#~ "PEJMAN5\tPEJMAN6\t1\t0\t1\t0\t0.83\t0.5\n";
	...
	foreach my $fields_tok(sort keys %{$hash1{$CHROM_POS_tok}{$REF_ALT_tok}{$SYMBOL_tok}})
	{
		...
		print OUTPUT"$CHROM_POS_tok\t$REF_ALT_tok\t$SYMBOL_tok\t$fields_tok\t";
